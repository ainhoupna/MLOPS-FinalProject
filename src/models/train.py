...
class FraudDetectionTrainer:
    ...
    def train(
        ...
        with mlflow.start_run(run_name="optuna_optimization"):
            ...
            optimal_threshold, best_f1 = self.find_optimal_threshold(y_val, y_val_pred_proba)
            print(f"Optimal threshold: {optimal_threshold:.4f} (F1: {best_f1:.4f})")
            
            # Calculate metrics
            val_metrics = self.calculate_metrics(y_val, y_val_pred_proba, optimal_threshold)
            ...
            self.plot_confusion_matrix(y_val, y_val_pred, "artifacts/confusion_matrix.png")
            ...
            X_sample = X_val.sample(min(1000, len(X_val)), random_state=self.random_state)
...
def main():
    ...
    test_metrics = trainer.calculate_metrics(y_test, y_test_pred_proba, optimal_threshold)
